...
function setClasslist_cm(clid, clname, cllevel, cltype, price30, price60) {   
    ...
    class_div.setAttribute("class", "class-div ml-4 mr-4 bg-gray-50 justify-between rounded-lg my-2 border-2 hover:shadow");
    // 디폴트 value값으로 0 세팅
    ...
    class_div.addEventListener('click', function() {
        ...
        for (const label of all_class_div) {
            ...
            label.setAttribute("class", "class-div ml-4 mr-4 bg-gray-50 justify-between rounded-lg my-2 border-2 hover:shadow");
            // 모든 수업 value값 0으로
        ...
        class_div.setAttribute("class", "class-div ml-4 mr-4 bg-gray-200 justify-between rounded-lg my-2 border-2 hover:shadow");
        // 선택한 수업의 value의 값을 1로 설정
        class_div.setAttribute("value", '1');

        // checkNextbtn_cl(all_class_div);
        console.log(class_div.value);
    })
}

// 다음 버튼 활성화 여부
    // function checkNextbtn_cl(all_class_div) {

    //     for (const label2 of all_class_div) {
    ...
    //         nextBtn_cl.setAttribute("class", "nextBtn_cl bg-gray-200 px-3 py-1 rounded text-white");
    //     }
    //     else {
    //         nextBtn_cl.setAttribute("class", "nextBtn_cl bg-blue-500 hover:bg-blue-600 px-3 py-1 rounded text-white");
    //     }
    // }
