...
//controller
export async function controller(model, view){
    ...
    // 초기 세팅 (유저 id, 타임존, 수업 목록 가져오기)
    const init = async function() {
        
        console.log(model);
        // model.get_utc();
        // model.getSchedulesList();
    }
    ...
    // 이전, 다음 버튼 이벤트 리스너 선언
    prev_btn.addEventListener('click', reRender);
    next_btn.addEventListener('click', reRender);
