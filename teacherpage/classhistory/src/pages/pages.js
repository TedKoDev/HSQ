...
export const changeSelectBtnStyle = (target, $historyType) => {
    ...
    for (let i = 0; i < classType.length; i++) {
        classType[i].setAttribute("class", "historyType mx-2 py-5 font-normal text-sm border-0");

        // console.log("change_pass");
    }
target.setAttribute("class", "historyType mx-2 py-5 font-semibold text-sm border-b-4 border-blue-400 text-gray-800")
};

export const getFilterblock = ($filter) => {    
    
    $filter.innerHTML = `<div class = "flex w-full">
  ...
  <input class = "inputName w-1/6 text-sm px-1 py-1 rounded border border-gray-200 mb-3 mx-1" placeholder = "학생명"></input>
  ...
  <button class=" bg-blue-500 hover:bg-blue-600 text-white rounded-lg w-1/6 h-8
