...
// // $kind = 'teacher'; //kind
// $kind = 'student'; //kind
// $teacher_review = '텍스트리뷰12345' ; //teacher_review 텍스트 
// $student_review = '학생텍스트리뷰12345' ; //teacher_review 텍스트 
...
if($kind == 'teacher'){
 ...
 if ($response) { //정상적으로 파일 저장되었을때 
  ...
  $send["class_register_id"]   =  'Teacher_Schedule 업뎃완료';
  $send["teacher_review"]   =  $teacher_review;
  $send["success"]   =  "no";
  echo json_encode($send);
 ...
 if ($response) { //정상적으로 파일 저장되었을때 
  ...
  $select1 = "INSERT INTO Class_Student_Review (class_register_id, student_review, student_review_star, student_review_date) VALUES ('$class_register_id', '$student_review','$student_review_star', now()) ";
  $response1 = mysqli_query($conn, $select1);
  mysqli_close($conn);
